...
impl TokioRaft {
  ...
  pub fn leadership_watcher(&self) -> TokioLeadershipWatcher {
    TokioLeadershipWatcher(self.0.leadership_watcher())
  }
  ...
  pub fn leadership_change_watcher(&self) -> TokioLeadershipWatcher {
    TokioLeadershipWatcher(self.0.leadership_change_watcher())
  }
  ...
  pub fn apply<'a>(&'a self, py: Python<'a>, data: RaftData) -> PyResult<&'a PyAny> {
    ...
    TokioRuntime::into_supported().future_into_py(py, async move {
      Ok(TokioApplyFuture::from(this.apply(data).await))
    })
  }
  ...
  pub fn barrier<'a>(&'a self, py: Python<'a>) -> PyResult<&'a PyAny> {
    let this = self.0.clone();
    TokioRuntime::into_supported().future_into_py(py, async move {
      Ok(TokioBarrierFuture::from(this.barrier().await))
    })
  }
  ...
  pub fn verify_leader<'a>(&'a self, py: Python<'a>) -> PyResult<&'a PyAny> {
    let this = self.0.clone();
    TokioRuntime::into_supported().future_into_py(py, async move {
      Ok(TokioVerifyFuture::from(this.verify_leader().await))
    })
  }
  ...
  pub fn add_voter<'a>(
    ...
    let this = self.0.clone();
    TokioRuntime::into_supported().future_into_py(py, async move {
      Ok(TokioMembershipChangeFuture::from(
  ...
  pub fn shutdown<'a>(&'a self, py: Python<'a>) -> PyResult<&'a PyAny> {
    let this = self.0.clone();
    TokioRuntime::into_supported().future_into_py(py, async move { Ok(this.shutdown().await) })
  }
  ...
  pub fn snapshot<'a>(&'a self, py: Python<'a>) -> PyResult<&'a PyAny> {
    let this = self.0.clone();
    TokioRuntime::into_supported().future_into_py(py, async move {
      Ok(TokioSnapshotFuture::from(this.snapshot().await))
    })
  }
  ...
  pub fn leadership_transfer<'a>(&'a self, py: Python<'a>, id: NodeId) -> PyResult<&'a PyAny> {
    let this = self.0.clone();
    TokioRuntime::into_supported().future_into_py(py, async move {
      Ok(TokioLeadershipTransferFuture::from(
